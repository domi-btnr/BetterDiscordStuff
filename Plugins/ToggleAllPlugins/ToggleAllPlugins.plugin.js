           ...
           const plugin = (Plugin, Library) => {
               ...
               return class ToggleAllPlugins extends Plugin {
                   ...
                   getSettingsPanel(){
                       ...
                       const doneButton = document.getElementsByClassName("bd-button button-38aScr lookFilled-1Gx00P colorBrand-3pXr91 sizeMedium-1AC_Sl grow-q77ONN");
           
                       const enablePlugins = document.createElement("button");
                       enablePlugins.className = "button-38aScr lookFilled-1Gx00P colorBrand-3pXr91 sizeMedium-1AC_Sl grow-q77ONN";
                       enablePlugins.style = "margin-right: 8px;";
                       enablePlugins.addEventListener("click", ()=>{
                           if(this.settings.closeSettingsModal){
                               doneButton[0].click()
                           }
                           console.log(`%c${config.info.name}`, "background: #e91e63; color: white; padding: 2px; border-radius: 4px; font-weight: bold;", "Enabling all Plugins");
                       ...
                       disablePlugins.className = "button-38aScr lookFilled-1Gx00P colorRed-1TFJan sizeMedium-1AC_Sl grow-q77ONN";
                       disablePlugins.style = "margin-right: 8px;";
                       disablePlugins.addEventListener("click", ()=>{
                           if(this.settings.closeSettingsModal){
                               doneButton[0].click()
                           }
                           console.log(`%c${config.info.name}`, "background: #e91e63; color: white; padding: 2px; border-radius: 4px; font-weight: bold;", "Disabling all Plugins");
                       ...
                       new Settings.SettingGroup("General", {shown:true}).appendTo(panel)
                       .append(
                           new Settings.Switch("Reload Discord", "Reloads Discord after the Plugins got toggled", this.settings.reloadDiscord, checked => {
                               if (checked){
                       ...
                       .append(
                           new Settings.Switch("Close Plugin Settings", "Close the Settings Modal after the Plugins got toggled", this.settings.closeSettingsModal, checked => {
                               if (checked){
                       ...
                       .append(
                           new Settings.Textbox("Ignore Plugins", "Put the name of all plugins that should get ignored when you toggle all Plugins in this textbox separated by a comma", this.settings.pluginsToIgnore.join(","), val=>{
                               this.settings.pluginsToIgnore = val.split(",").map(x=>x.trim()).filter(x=>x);
                               this.saveSettings();
                           })
                       )
