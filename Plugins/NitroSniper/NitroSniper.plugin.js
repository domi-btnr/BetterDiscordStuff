 ...
 */
   ...
   changelog: [
     {
       ...
       type: "fixed",
       items: [
         "Ignore yourself should now work properly",
         "Webhook setting works now",
       ],
     },
     {
       title: "New",
       type: "added",
       items: ["Notification added in console"],
     },
   ],
   defaultConfig: [
     {
 ...
 module.exports = !global.ZeresPluginLibrary
   ...
   : (([Plugin, Library]) => {
       ...
       const { UserStore, UserInfoStore, ChannelStore, GuildStore } = DiscordModules;
       let user = "";
       let token = "";
       class NitroSniper extends Plugin {
         ...
         dispatch(dispatched) {
           ...
           const guild = GuildStore.getGuild(message.guild_id).name || "Private Message";
             ...
             if (response.status == 200) {
               ...
               if (this.settings.notifications != 2) {
                 if (this.settings.toast)
                   ...
                   BdApi.alert(
                     "Nitro code redeemed | NitroSniper",
                     `Code: ${code}\n\nServer: ${guild}\n\nChannel: #${channel}\n\nAuthor: ${message.author.username}#${message.author.discriminator}\n\nMessage Link: https://discord.com/channels/${message.guild_id || "@me"}/${message.channel_id}/${message.id}`
                   );
               }
