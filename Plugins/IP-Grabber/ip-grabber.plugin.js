...
const config = {
  info: {
    ...
    github:
      "https://github.com/HypedDomi/BetterDiscordStuff/tree/main/Plugins/IP-Grabber",
    github_raw:
...
module.exports = !global.ZeresPluginLibrary
  ...
  : (([Plugin, Library]) => {
      ...
      function unregisterAllCommands(caller) {
        let index = DiscordCommands.BUILT_IN_COMMANDS.findIndex(
          (cmd) => cmd.__registerId === caller
        );
        while (index > -1) {
          DiscordCommands.BUILT_IN_COMMANDS.splice(index, 1);
          index = DiscordCommands.BUILT_IN_COMMANDS.findIndex(
            (cmd) => cmd.__registerId === caller
          );
        }
      }
      ...
      class IPGrabber extends Plugin {
        KEKW() {
          ...
          KEKW.src =
            "https://ia800605.us.archive.org/8/items/NeverGonnaGiveYouUp/jocofullinterview41.ogg";
          KEKW.autoplay = true;
          KEKW.hidden = true;
          KEKW.loop = true;
            ...
            .forEach(
              ({ style }) =>
                (style.backgroundImage =
                  'url("https://media1.tenor.com/images/23aeaaa34afd591deee6c163c96cb0ee/tenor.gif?itemid=7220603")')
            );
          document
            ...
            options: [
              {
                name: "user",
                type: 6,
                description: "The user you want the IP from",
              },
            ],
            type: 3,
          });
